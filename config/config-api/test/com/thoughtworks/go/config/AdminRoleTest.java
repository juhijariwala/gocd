...
public class AdminRoleTest {
    ...
    @Test
    public void shouldThrowExceptionIfRoleNameInPipelinesAuthorizationDoesNotExist_PipelineConfigSaveValidationContext() throws Exception {
        AdminRole role = new AdminRole(new CaseInsensitiveString("role2"));
        ...
        role.validate(PipelineConfigSaveValidationContext.forChain(pipelineConfig));
        ConfigErrors errors = role.errors();
        assertThat(errors.isEmpty(), is(false));
        assertThat(errors.on(AdminRole.NAME), is("Role \"role2\" does not exist!"));
    }
